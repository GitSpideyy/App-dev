...
try {
    ...
    if (isset($_POST["studentno"]) && isset($_POST["firstname"]) && isset($_POST["middlename"]) && isset($_POST["lastname"]) && isset($_POST["contact"]) && isset($_POST["email"]) && isset($_POST["birthdate"])) {
        ...
        $stmt = $conn->prepare("INSERT INTO student (studentno, firstname, middlename, lastname, contact, email, birthdate) VALUES (:studentno, :firstname, :middlename, :lastname, :contact, :email, :birthdate)");
        ...
        if ($stmt->execute()) {
            ...
            echo json_encode(array("response" => "error", "message" => "Account creation failed"));
        }
